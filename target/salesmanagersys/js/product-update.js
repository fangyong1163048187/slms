$(function(){
    ...
    var buyprice = true;
    var salesprice = true;
    var stockamount = true;
    var salesamount = true;
    //检验编号
    $("#number").bind("mouseover click input",function () {
        if(!$(this).val().match(/^PD\d{18}$/)){
    ...
    //检验是否可以提交数据
    $("#sub").click(function () {
        if(number===false || name===false || buyprice===false || salesprice===false || stockamount===false || salesamount===false){
            return false;
        }else{
            var data = $(".form").serializeArray();
            $.ajax({
                url:"product-update-after",
                type:"post",
                data:data,
                dataType:"json",
                success:function (data) {
                    if(data.row!=null){
                        ...
                        var name = $("#name").val();
                        window.location.href="product_select?product_id="+number+"&product_name="+name;
                    }else{
                        alert("更改数据失败");
                    }
                }
